...
var UserRels = struct {
	Avatar                 string
	IssuedTokens           string
	PosterMessageReactions string
	SenderMessages         string
}{
	Avatar:                 "Avatar",
	IssuedTokens:           "IssuedTokens",
	PosterMessageReactions: "PosterMessageReactions",
...
type userR struct {
	...
	PosterMessageReactions MessageReactionSlice `db:"PosterMessageReactions" boil:"PosterMessageReactions" json:"PosterMessageReactions" toml:"PosterMessageReactions" yaml:"PosterMessageReactions"`
...
// PosterMessageReactions retrieves all the message_reaction's MessageReactions with an executor via poster_id column.
func (o *User) PosterMessageReactions(mods ...qm.QueryMod) messageReactionQuery {
	var queryMods []qm.QueryMod
	if len(mods) != 0 {
		queryMods = append(queryMods, mods...)
	}
	...
	queryMods = append(queryMods,
		qm.Where("\"message_reactions\".\"poster_id\"=?", o.ID),
	)
...
// LoadPosterMessageReactions allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (userL) LoadPosterMessageReactions(e boil.Executor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	var slice []*User
	var object *User
	...
	query := NewQuery(
		...
		qm.WhereIn(`message_reactions.poster_id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	if singular {
		object.R.PosterMessageReactions = resultSlice
		for _, foreign := range resultSlice {
			if foreign.R == nil {
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if local.ID == foreign.PosterID {
				local.R.PosterMessageReactions = append(local.R.PosterMessageReactions, foreign)
				if foreign.R == nil {
					foreign.R = &messageReactionR{}
				}
...
// AddPosterMessageReactions adds the given related objects to the existing relationships
// of the user, optionally inserting them as new records.
// Appends related to o.R.PosterMessageReactions.
// Sets related.R.Poster appropriately.
func (o *User) AddPosterMessageReactions(exec boil.Executor, insert bool, related ...*MessageReaction) error {
	var err error
	for _, rel := range related {
		if insert {
			rel.PosterID = o.ID
	...
	if o.R == nil {
		o.R = &userR{
			PosterMessageReactions: related,
		}
	} else {
		o.R.PosterMessageReactions = append(o.R.PosterMessageReactions, related...)
	}
