...
type Blob struct {
	...
	FileName      string    `db:"file_name" boil:"file_name" json:"fileName" toml:"fileName" yaml:"fileName"`
	MimeType      string    `db:"mime_type" boil:"mime_type" json:"mimeType" toml:"mimeType" yaml:"mimeType"`
	FileSizeBytes int64     `db:"file_size_bytes" boil:"file_size_bytes" json:"fileSizeBytes" toml:"fileSizeBytes" yaml:"fileSizeBytes"`
...
var BlobRels = struct {
	...
	AvatarHubs:  "AvatarHubs",
	AvatarUsers: "AvatarUsers",
}

// blobR is where relationships are stored.
type blobR struct {
	AvatarHubs  HubSlice  `db:"AvatarHubs" boil:"AvatarHubs" json:"AvatarHubs" toml:"AvatarHubs" yaml:"AvatarHubs"`
...
var (
	blobAllColumns            = []string{"id", "file_name", "mime_type", "file_size_bytes", "extension", "file", "deleted_at", "updated_at", "created_at"}
	blobColumnsWithoutDefault = []string{"file_name", "mime_type", "file_size_bytes", "extension", "file", "deleted_at"}
	blobColumnsWithDefault    = []string{"id", "updated_at", "created_at"}
...
// AvatarHubs retrieves all the hub's Hubs with an executor via avatar_id column.
func (o *Blob) AvatarHubs(mods ...qm.QueryMod) hubQuery {
...
// LoadAvatarHubs allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (blobL) LoadAvatarHubs(e boil.Executor, singular bool, maybeBlob interface{}, mods queries.Applicator) error {
	var slice []*Blob
	var object *Blob
	...
	query := NewQuery(
		qm.From(`hubs`),
		qm.WhereIn(`hubs.avatar_id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	if singular {
		object.R.AvatarHubs = resultSlice
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if queries.Equal(local.ID, foreign.AvatarID) {
				local.R.AvatarHubs = append(local.R.AvatarHubs, foreign)
				if foreign.R == nil {
					foreign.R = &hubR{}
				}
...
// AddAvatarHubs adds the given related objects to the existing relationships
// of the blob, optionally inserting them as new records.
// Appends related to o.R.AvatarHubs.
// Sets related.R.Avatar appropriately.
func (o *Blob) AddAvatarHubs(exec boil.Executor, insert bool, related ...*Hub) error {
	var err error
	...
	if o.R == nil {
		...
		o.R.AvatarHubs = append(o.R.AvatarHubs, related...)
	}
...
// SetAvatarHubs removes all previously related items of the
// blob replacing them completely with the passed
// in related items, optionally inserting them as new records.
// Sets o.R.Avatar's AvatarHubs accordingly.
// Replaces o.R.AvatarHubs with related.
// Sets related.R.Avatar's AvatarHubs accordingly.
func (o *Blob) SetAvatarHubs(exec boil.Executor, insert bool, related ...*Hub) error {
	query := "update \"hubs\" set \"avatar_id\" = null where \"avatar_id\" = $1"
	...
	if o.R != nil {
		for _, rel := range o.R.AvatarHubs {
	...
	return o.AddAvatarHubs(exec, insert, related...)
}

// RemoveAvatarHubs relationships from objects passed in.
// Removes related items from R.AvatarHubs (uses pointer comparison, removal does not keep order)
// Sets related.R.Avatar.
func (o *Blob) RemoveAvatarHubs(exec boil.Executor, related ...*Hub) error {
	var err error
	for _, rel := range related {
		queries.SetScanner(&rel.AvatarID, nil)
		if rel.R != nil {
	...
	for _, rel := range related {
		for i, ri := range o.R.AvatarHubs {
			if rel != ri {
				continue
			}

			ln := len(o.R.AvatarHubs)
			if ln > 1 && i < ln-1 {
				o.R.AvatarHubs[i] = o.R.AvatarHubs[ln-1]
			}
			o.R.AvatarHubs = o.R.AvatarHubs[:ln-1]
			break
		}
	}
