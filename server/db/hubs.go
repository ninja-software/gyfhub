...
var HubWhere = struct {
	...
	OwnerID:   whereHelperstring{field: "\"hubs\".\"owner_id\""},
...
var (
	hubAllColumns            = []string{"id", "owner_id", "name", "deleted_at", "updated_at", "created_at"}
	hubColumnsWithoutDefault = []string{"owner_id", "name", "deleted_at"}
	hubColumnsWithDefault    = []string{"id", "updated_at", "created_at"}
...
func (o *Hub) Owner(mods ...qm.QueryMod) userQuery {
...
func (hubL) LoadOwner(e boil.Executor, singular bool, maybeHub interface{}, mods queries.Applicator) error {
	var slice []*Hub
	var object *Hub
	...
	if singular {
		...
		foreign.R.OwnerHubs = append(foreign.R.OwnerHubs, object)
		return nil
	}
	...
	for _, local := range slice {
		for _, foreign := range resultSlice {
			if local.OwnerID == foreign.ID {
				...
				foreign.R.OwnerHubs = append(foreign.R.OwnerHubs, local)
				break
			}
		}
	}
...
// SetOwner of the hub to the related item.
// Sets o.R.Owner to related.
// Adds o to related.R.OwnerHubs.
func (o *Hub) SetOwner(exec boil.Executor, insert bool, related *User) error {
	var err error
	if insert {
	...
	if related.R == nil {
		related.R = &userR{
			OwnerHubs: HubSlice{o},
		}
	} else {
		related.R.OwnerHubs = append(related.R.OwnerHubs, o)
	}
